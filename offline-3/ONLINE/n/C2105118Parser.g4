...
statements returns [std::string unit_name, int line_number, bool retError= false]
	   ...
	   | sts=statements statement {
		...
		DO cs=compound_statement WHILE LPAREN expression RPAREN {
			...
			$line_number = $cs.line_number;
			std::string message = "Line " + std::to_string($statement.line_number) + ": statements : statements DO LCURL statement RCURL WHILE LPAREN expression RPAREN";
			message += "\n\n" + $unit_name;
			cout<< "DO "<<endl;
			writeIntoparserLogFile(message, 2);
		}
	  ...
	  | RETURN expression sm=SEMICOLON {
		...
		BREAK sm=SEMICOLON {
			if (inLoop==1){
				...
				$unit_name = "statement " + $expression.var_name + ";";
				$line_number = $sm->getLine();
				message += "\n\n" + $unit_name;
				writeIntoparserLogFile(message, 2);
			}
		...
		CONTINUE sm=SEMICOLON {
			if (inLoop==1){
				...
				std::string message = "Line " + std::to_string($BREAK->getLine()) + ": Continue outside loop";
				$unit_name = "break" ";";
				$line_number = $sm->getLine();
				message += "\n\n" + $unit_name;
				writeIntoparserLogFile(message, 2);
